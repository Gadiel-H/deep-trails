...
function typeOf(value, strLength = "full") {
    if (strLength === undefined || strLength === null)
        strLength = "full";
    const length = String(strLength).trim().toLowerCase();
    if (length !== "full" && length !== "short")
        throw new Error("Invalid value for the length parameter: " + strSimple(length)
            + "\n\tExpected: " + '"short" | "full"');
